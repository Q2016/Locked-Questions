...
Another example: let's say we roll dice i = 5 times, and the last face is j = 5, then we have this sequence 
xxxx5 where x could ranges from 0 to 5 arbitrarily if we are not restricted. Then let's calculate how many 
combinations we could form xxxx5 with constraint rollMax[j] = 3. They could be categorized as follow:
...
Then the question is how to calculate the combinations of ...xy. Actually if you take a deeper look, what ...xy 
means is essentially: give me all the combinations as long as the last face is not 5. We don't care what x should 
be here because it is not restricted and could choose any value (it could be even same as y, or same as 5). 
As long as y is not equal to 5, we are good to go. And that is the definition dp[i][j]!
