...
zsource such that their concatenation equals target. If the task is impossible, return -1.
...
then we will improve our algorithm to O(n log n) running time; finally we will further improve the algorithm to O(n).
...
If the character exists in source after position i (e.g. exists at position j where j>i), we move on to the next character, and set i to the value of j;
...
If the character does not exists in source after position i, and i is not 0, we increment the output count by 1, and set i to 0, and then repeat from step 1.
