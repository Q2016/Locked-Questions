...
parts by ensuring that totalArraySum % k is equal to 0. Now, if the array sum can be evenly divided into k parts, as previously mentioned, 
we will try to build those k subsets using backtracking. We will keep a currentSum variable denoting the sum of the current subset. One by one, 
try to include each element from the array that has not already been picked and then make a recursive call to pick the next element.
...
element by setting taken[i] equal to false so that it can be picked in a future recursive call. If we reach the condition currentSum is greater 
than targetSum, then we cannot reach the target by adding more elements to the subset, so there is no need to proceed further; we can just 
backtrack from here. If we reach the condition, currentSum equals targetSum, that means we made one subset with the target sum. So 
